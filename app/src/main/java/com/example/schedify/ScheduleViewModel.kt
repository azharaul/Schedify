...
class ScheduleViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val sharedPreferences = application.getSharedPreferences("schedify_prefs", Context.MODE_PRIVATE)
    ...
    fun initializeDefaultSchedules() {
        viewModelScope.launch {
            // Check if default schedules have already been initialized
            val isInitialized = sharedPreferences.getBoolean("default_schedules_initialized", false)
            ...
            if (!isInitialized) {
                ...
                allSchedules.first().let { schedules ->
                    if (schedules.isEmpty()) {
                        ...
                        // Mark default schedules as initialized to prevent future re-initialization
                        sharedPreferences.edit().putBoolean("default_schedules_initialized", true).apply()
                    }
                }
            }
        }
